...
Figures below are **reproduced verbatim** from `python -m engine.fusion.eval`, at the precision the
tool emits. Rounding in the document and full precision in the tool is a small disagreement, and a
small disagreement is still one an opposing expert can point at.
...
| F1 @ τ(α=0.05) | **0.9381** |
| **False-merge rate @ τ** | **0.031348 ≤ α = 0.05** (3.1%) ✔ |
